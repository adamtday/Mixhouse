...
retrieval_tags: [progressive_disclosure, site_objectives, audience, sitemap]
related:
...
- The site reveals depth in four layers: Atmosphere → Story → Substance → Gated.
- Each layer corresponds to user intent and audience type.
- Friction is intentional: it filters audiences toward the surfaces designed for them.
- The model maps directly to the sitemap (`08_website_strategy/sitemap.md`).
...
- One CTA (typically the trailer or signup, depending on phase).
- No long-form copy.
...
- Owner reviews each inquiry before issuing materials.
- Tracked via CRM.

### Visitor Routing

- Inbound from social or fan press → L1 → L2.
- Inbound from broadcaster trade press → L1 → L2 → L3 → L4.
- Inbound from advisor referral (investor) → directly to L3 / L4 link via outreach.
- Inbound from casting partner → L1 → The Show casting CTA.

### Implementation

- L1 + L2 + L3 are publicly indexed.
- L4 is noindexed and requires form completion + manual approval before delivery.
- CRM tags each visitor by entry point and audience.
- No tracking beyond consented analytics.

### Anti-Pattern: Premature Substance

Surface-level visitors shown financial models will:
- Misquote numbers.
- Anchor expectations badly.
- Reduce optionality in future fundraises.

Anti-Pattern: Withholding Atmosphere

Investors-only visitors hate dry decks. Even the Materials gate should preserve the brand world.
...
- Every page brief references its layer in this model.
- The Materials brief carries the gate flow specifically.
