...
retrieval_tags: [judges, judging, format_engine, elimination_logic, cultural_authenticity]
related:
...
- One must be an established producer / artist with peer respect.
- One must come from the A&R / label / promoter side.
- A third (where used) brings a sound or region not covered by the first two.
- Diversity across gender, age, and scene origin is mandatory.

### The Four Criteria

Every track is evaluated on:
...
The `Audience Index` (see `02_show_bible/elimination_logic.md`) combines:

| Input | Weight (initial proposal) |
| --- | --- |
| Resident Judge panel | 40% |
| `Drop Night` live audience reaction | 20% |
| Streaming performance (first 72h) | 25% |
| Short-form virality | 15% |

Final weights to be calibrated in pilot.
...
- Judges with formal label, agency, or management ties to a contestant must declare and recuse from that contestant's scoring.
- All recusals are filmed and shown on screen.
- A "Floating Judge" rotation can cover recusals without breaking the panel cadence.

### Mentor Drops

- Mentors do *not* vote.
- Their role is on-camera craft mentorship and reputational endorsement.
- Mentor selection follows the same scene-credibility-first principle.

## Evidence / Sources

| # | Claim | Source | Date accessed | Reliability |
| - | --- | --- | --- | --- |
| 1 | Scene-credible judging precedent (`Rhythm + Flow` judging panel) | `04_competitive_landscape/rhythm_and_flow.md` | 2026-05-20 | B |
...
- Judge bios on the public site emphasise scene credentials, not TV credits.
- The four criteria appear publicly on `the_show.md` as a "how it's judged" component.

## Open Questions

- Final weighting of inputs (to be pilot-tested).
- Whether a judge "veto" exists for unsafe / non-releasable tracks.
- Format for declaring conflicts of interest on-screen.
